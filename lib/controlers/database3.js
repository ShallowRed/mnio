...
const CreateGridTable = "CREATE TABLE game_?__grid (orderid INT PRIMARY KEY NOT NULL AUTO_INCREMENT, cellid MEDIUMINT, playerid SMALLINT NOT NULL, color VARCHAR(6) NOT NULL)";
...
const save = {
  ...
  fill: (cellid, name, playerid, color) => {
    db.query(GetGameid, [GAMEDATE], (err, res) => {
      if (err) throw err;
      const gameid = res[0].gameid;
      db.query(AddCelltoGrid, [gameid, cellid, playerid, color], err => {
        if (err) throw err;
        ...
        // console.table(name + playerid + cellid + color + gameid);
