...
const SQL = {
  ...
  query: (query, el, cb) => {
    ...
    const sqlQuery = SQL.queries[query];
    db.query(sqlQuery, el, (err, response) => {
      if (!!err) throw err;
...
SQL.queries = {
  ...
  newGame: "INSERT INTO games (`usedrows`, `usedcols`, `gridid` ,`flag`) VALUES(?, ?, ?, 0)",
  addCelltoGrid: "INSERT INTO game_?__grid (`cellid`, `playerid`, `color`)  VALUES(?, ?, ?)",
  saveUserPalette: "INSERT INTO game_?__players (`playerid`, `color1`, `color2`, `color3`, `color4`, `color5`) VALUES(?, ?, ?, ?, ?, ?)",
  updateFlag: "UPDATE games SET flag = ? WHERE gameid = ?",
  addUsertoUsers: "INSERT INTO users (`Username`, `Password`) VALUES(?, ?)",
...
const init = ColorList => {
  ...
  SQL.query("getLastGame", null, res => {
    if (!res.length || res[0].flag) newGame();
    else lastGame(res[0].gameid, ColorList);
  })
}

const newGame = () => {
  SQL.query("newGame", [Config.rows, Config.cols, GAMEDATE], () =>
...
const lastGame = (gameid, ColorList) => {
