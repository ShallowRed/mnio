...
function getgames(socket) {
  db.query(Testdb, (err, res) => {
    if (!!err) throw err;
    let games = [];
    res.forEach( game => games.push([game.gameid, game.usedrows, game.usedcols, game.flag]));
    User.sendgames(socket, games);
  });
}
...
function gettable(socket, gameid) {
  db.query(SelectGrid, [parseInt(gameid)], (err, res)  => {
    if (!!err) throw err;
    let cells = [];
    res.forEach(cell => cells.push([cell.cellid, '#' + cell.color]));
    User.sendtable(socket, cells);
  });
}
  ...
  db.query(IsUserinUsers, [user], (err, res1, fields) => {
    ...
    if (!res1.length) { // If user is not in database
      db.query(AddUsertoUsers, [user, pass], (err, result) => {
        if (!!err) throw err;
        User.init([result.insertId, user, socket], MNIO); // req.session.userID = result.insertId; req.session.save();
      });
      return;
    }
