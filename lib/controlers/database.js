...
const log = (user, pass, socket, MNIO) => {
  db.query(IsUserinUsers, [user], (err, res1, fields) => {
    // If user is not in database
    if (!res1.length) newPlayer(user, pass, socket, MNIO);
    // If wrong password
    else if (pass !== res1[0].Password) Player.wrongPass(socket);
    // If user is in database and right pass
    else returningPlayer(res1[0].playerid, user, socket, MNIO);
  });
};
