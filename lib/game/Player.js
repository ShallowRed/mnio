...
module.exports = {
  ...
  getExisting: async ({ userName, position, playerid }, { Database, Map }) => {
    ...
    const props = { playerid, position, userName, palette, ownCells };
...
class Player {
  ...
  constructor(params, Map) {
    const { playerid, userName, position, palette, ownCells } = params;
    ...
    this.position = position || this.ownCells[0] || randomPosition(gridState)
