...
module.exports = {
  ...
  getPlayerDeeds: async function(playerid) {
    ...
    return {
      ...
      ownCells: player.length ? await getPlayerOwned.call(this, player, playerid) :
        null
    }
  }
};

const getPlayerOwned = async function(player, playerid) {
  const ownCells = await sQuery("isUserinGrid", [this.gameid, playerid]);
