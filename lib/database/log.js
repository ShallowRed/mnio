...
const log = {
  ...
  pass: (user, pass, socket, MNIO) =>
    SQL.query("isUserinUsers", [user], res => {
      if (!res.length) newPlayer(user, pass, socket,
      MNIO); // If user is not in database
      else if (pass !== res[0].Password) Player.wrongPass(
      socket); // If wrong password
      else returningPlayer(res[0].playerid, user, socket,
      MNIO); // If user is in database and right pass
    }),
}
...
const newPlayer = (user, pass, socket, MNIO) =>
  SQL.query("saveUser", [user, pass], res => {
    ...
    Player.log(playerid, user, socket, MNIO);
  ...
  SQL.getGameId(gameid =>
    SQL.query("isUserinPlayers", [gameid, playerid], player => {
      if (!player.length)
        ...
        SQL.query("isUserinGrid", [gameid, playerid], owning => {
          const colors = Object.keys(player[0])
            .filter(e => e[0] == "c")
            .map(e => '#' + player[0][e]);
          const owncells = owning.map(e => e.cellid)
          Player.log(playerid, user, socket, MNIO, colors, owncells);
