module.exports = {
  ...
  movePlayer(direction) {
    const {socket, player, Game} = this;
    socket.emit("moveCallback");
    const newPos = player.checkMove(direction, Game);
    if (!newPos) return;
  ...
  saveFill(cell) {
    const {socket, player, Game, Database} = this;
    ...
    player.updateOwnings(cell.position)
    socket.emit('AllowedCells', player.allowedcells);
  },

  disconnect() {
    const {socket, player: { name, dbid, position, colors }, Game} = this;
