...
async function initPlayer(password, context) {
  ...
  userInDb.exists ?
    ...
    alertEnd(socket) :
    newPlayer(context, { userName, password });
}

async function returningPlayer(context) {
  const { socket, userInDb, userName, network } = context;
  const { Database, Game } = network;
...
async function newPlayer(context, { userName, password }) {
  const { socket, network } = context;
  network.players[socket.request.sessionId] = { userName, password };
}
