...
async function initPlayer(input) {
  const { socket, userInDb, network: { Game } } = this;
  socket.emit('isNew', !userInDb.exists);
  userInDb.exists ?
    returningPlayer.call(this, input) :
    !Game.hasAvailableSpace() ?
    alertEnd.call({ socket }) :
    socket.on("paletteSelected", newPlayer.bind(this, input));
}
...
async function newPlayer([userName, password], index) {
  ...
  const palette = await Database.savePalette(playerid, index);
