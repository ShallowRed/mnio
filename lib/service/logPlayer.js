...
async function checkUsername(userName) {
  ...
  const context = { socket, network, userInDb };
...
async function initPlayer(creds) {
  const { socket, userInDb, network: { Database, Game } } = this;
  ...
  if (userInDb.exists) {
    ...
    return;
  } else !Game.hasAvailableSpace() ?
    alertEnd.call({ socket }) :
    socket.on("paletteSelected", saveAndSpawnPlayer.bind(this, creds));
}

async function saveAndSpawnPlayer([userName, password], index) {
  const { socket, network: { Database, Game: { gridState } } } = this;

  const playerid = await Database.saveCredentials(userName, password);
  const palette = await Database.savePalette(playerid, Pokedex[index]);
