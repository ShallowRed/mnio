const update = (APP) => {
  ...
  APP.canvas.style.marginTop = (APP.dvflag && w < h) ?
    "60px" : (h - length) / 2 + "px";

  APP.canvas.style.marginLeft = APP.buttons.window.style.marginLeft =
    (APP.dvflag && w > h) ? "60px" : (w - length) / 2 + "px";

  APP.dataviz.style.marginTop = (APP.dvflag && w < h) ?
    length + 100 + "px" : (h - length) / 2 + "px";

  APP.dataviz.style.marginLeft = (APP.dvflag && w > h) ?
    length + 100 + "px" : (w - length) / 2 + "px";
