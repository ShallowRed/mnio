...
PLAYER.update = (GAME, MAP) => {
  ...
  PLAYER.is = {
    ...
    down: PLAYER.coord[0] > GAME.RowCol[0] - MAP.half[0],
    left: PLAYER.coord[1] < MAP.half[1],
    right: PLAYER.coord[1] > GAME.RowCol[1] - MAP.half[1]
  }
};

PLAYER.render = (animated, MAP, GAME) => {
  PLAYER.canvas.forEach(canvas => {
    canvas.style.transitionDuration = animated ? GAME.duration + 's' : '0s'
    canvas.style.top = (PLAYER.is.up ? PLAYER.coord[0] : PLAYER.is.down ? PLAYER.coord[0] + MAP.RowCol[0] - GAME.RowCol[0] - 2 : MAP.half[0] - 1) * MAP.cellSize + MAP.shift + 'px';
    canvas.style.left = (PLAYER.is.left ? PLAYER.coord[1] : PLAYER.is.right ? PLAYER.coord[1] + MAP.RowCol[1] - GAME.RowCol[1] - 2 : MAP.half[1] - 1) * MAP.cellSize + MAP.shift + 'px';
  });
  if (animated) return;
